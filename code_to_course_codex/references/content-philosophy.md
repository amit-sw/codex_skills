...
**Difficulty tracks:** Every module quiz should ship in three tracks: `low`, `medium`, and `deep`. They should all test the same concept family, but with different cognitive load:
- `low`: identify the right boundary or next step
- `medium`: apply the concept to a realistic change or bug
- `deep`: reason about tradeoffs, failure modes, or architecture choices

**How many quizzes:** One quiz container per module, placed at the end after the learner has seen all the content. The number of questions in each level track comes from intake and defaults to **5 per module**. Each question should make the learner pause and *think*, not just pick the obvious answer.

### Depth Should Change The Writing

The selected course depth changes the explanatory narrative across the whole course:
- `Low`: faster, more overview-oriented, fewer implementation branches
- `Medium`: balanced, practical, enough architecture and debugging detail to be useful
- `Deep`: denser tracing, more design rationale, more explicit failure modes

Do not create three versions of the entire module body. Only quizzes vary live in the browser. Explanation depth is fixed when the course is generated.
