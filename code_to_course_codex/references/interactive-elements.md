...
For testing understanding with instant feedback. In the Codex course shell, quizzes are **difficulty-aware**: every quiz container should include `low`, `medium`, and `deep` tracks, and the global course selector switches between them.

**Wiring:** `main.js` exposes `window.selectOption(btn)`, `window.checkQuiz(containerId)`, `window.resetQuiz(containerId)`, and a global quiz-level selector in the shell. Per-question explanations go in `data-explanation-right` and `data-explanation-wrong` on the `.quiz-question-block`.

**Authoring rule:** New Codex-generated courses should always emit three tracks. Legacy single-track markup still works, but it is only for backward compatibility.
...
<div class="quiz-container" id="quiz-module3">
  <div class="quiz-toolbar">
    <div>
      <span class="quiz-kicker">Practice</span>
      <p class="quiz-level-note">Current level: <span data-quiz-level-label>Medium</span></p>
    </div>
  </div>
  ...
  <div class="quiz-track" data-quiz-level="low">
    <div class="quiz-question-block"
         ...
         data-explanation-right="Exactly — this identifies the right system boundary."
         data-explanation-wrong="Not quite. Look for the component that owns this user-visible action.">
      <h3 class="quiz-question">Low-level question here?</h3>
  ...
  <div class="quiz-track active" data-quiz-level="medium">
    <div class="quiz-question-block"
      ...
      <h3 class="quiz-question">Medium-level question here?</h3>
      <div class="quiz-options">
        <button class="quiz-option" data-value="option-a" onclick="selectOption(this)">
          <div class="quiz-option-radio"></div>
          <span>Answer A</span>
  ...
  <div class="quiz-track" data-quiz-level="deep">
    <div class="quiz-question-block"
         ...
         data-explanation-right="Exactly — the tradeoff is worth it because of the failure mode described."
         data-explanation-wrong="Not quite. Consider the architecture tradeoff and what breaks under load or failure.">
      <h3 class="quiz-question">Deep-level question here?</h3>
      <div class="quiz-options">
        <button class="quiz-option" data-value="option-a" onclick="selectOption(this)">
          <div class="quiz-option-radio"></div>
          <span>Answer A</span>
...
**Rules:**
- Keep the same concept family across all three tracks, but scale the cognitive load
- `low` should test recognition and boundary-finding
- `medium` should test application to a realistic change or bug
- `deep` should test tradeoffs, failure modes, or architecture reasoning
- Put the same number of questions in each track unless you have a strong reason not to
...
Same HTML/CSS/JS pattern as Multiple-Choice Quizzes, including the three difficulty tracks. Wrap each question in a scenario context block:
...
**Variants:**
- `callout-accent`: accent border, light accent background (for CS insights)
- `callout-info`: info border, light info background (for "good to know")
