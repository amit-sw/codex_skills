...
### Missing Quiz Tracks
The Codex course shell expects every module quiz to support `low`, `medium`, and `deep` difficulty levels. If one module only ships a single question set, the global quiz selector becomes inconsistent and the course feels unfinished.

### Over-Asking During Intake
The skill should ask the high-impact brief questions up front, but only the missing ones. Re-asking for depth, audience, or quiz count after the user already specified them makes the workflow feel sloppy.
